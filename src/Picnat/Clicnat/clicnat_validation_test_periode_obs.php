...
class clicnat_validation_test_periode_obs extends clicnat_validation_test {
	...
	public function evaluer() {
		...
		$q = bobs_qm()->query($this->db, 's_select_period_obs', self::sql_select_periode, [$this->citation->id_espece, $decade]);
		$r = bobs_element::fetch($q);
		$passe = isset($r['decade']) && $r['decade'] == $decade;
		return [
			"passe" => $passe,
			"message" => $passe?"espèce déjà vu dans la décade {$decade}":"En dehors des périodes d'observation de l'espèce decade $decade"
		];
	}
}
